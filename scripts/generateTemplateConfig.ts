...
const buildConfigSource = (
  ...
  return [
    ...
    "components: {",
    ...(componentEntries.length > 0 ? componentEntries : ["// No components found in this template registry"]),
    "},",
...
const buildTemplateSource = (
  ...
  const importAnchor = 'import { AnalyticsProvider, SchemaWrapper } from "@yext/pages-components";';
  ...
  if (!baseSource.includes("export default Location;")) {
  ...
  return baseSource
    ...
    .replace(/\bconst Location\b/, `const ${templateComponentName}`)
    .replace("export default Location;", `export default ${templateComponentName};`);
};
