...
const buildConfigSource = (
  ...
  const configLines = [
    ...
    "components: {",
    ...(componentEntries.length > 0 ? componentEntries.map((entry) => {
      return `${entry},`;
    ...
    ...(categories ? [
      "categories: {",
      "components: {",
      `title: ${JSON.stringify(categories.components.title)},`,
      `components: ${JSON.stringify(categories.components.components)},`,
      "},",
      "},"
    ] : []),
    "};"
