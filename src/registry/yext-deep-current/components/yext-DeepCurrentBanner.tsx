...
type YextDeepCurrentBannerProps = {
  section: SectionTheme & {
    textColor?: ThemeColorValue;
  };
...
const BannerFields: YextFields<YextDeepCurrentBannerProps> = {
  section: {
    label: "Section",
    type: "object",
  ...
  content: {
    ...
    type: "object",
    objectFields: {
      bannerText: createEditableTextField("Banner Text"),
    },
  },
  styles: {
    label: "Style",
...
export const YextDeepCurrentBannerComponent: PuckComponent<YextDeepCurrentBannerProps> = (
  props,
) => {
  ...
  const bannerText = resolveText(
    content?.bannerText ?? props.bannerText,
    locale,
    streamDocument,
  ...
  const textColor = resolveThemeColor(
    props.styles?.textColor ?? props.section.textColor,
    "#425466",
  );
  ...
  return (
    ...
    >
      <AnalyticsScopeProvider
        name={`YextDeepCurrentBanner${getAnalyticsScopeHash(props.id)}`}
      >
        <section className="overflow-x-clip py-5" style={sectionStyles}>
          <div className="mx-auto max-w-[1410px] px-6">
            <div
              className="rounded-[16px] border border-black/5 px-6 py-4 text-center text-sm font-medium tracking-[0.02em] shadow-[0_6px_22px_rgba(9,30,66,0.06)]"
              style={{
...
export const YextDeepCurrentBanner: YextComponentConfig<YextDeepCurrentBannerProps> = {
  label: "Deep Current Banner",
  fields: BannerFields,
  defaultProps: {
    section: {
      backgroundColor: {
        selectedColor: "[#e8edf0]",
        contrastingColor: "palette-quaternary",
      },
      visibleOnLivePage: true,
    },
    content: {
      bannerText: createEditableText(
        "Planning for something new? Meet with an advisor for a personalized financial conversation.",
      ),
    },
    styles: {
  ...
  render: YextDeepCurrentBannerComponent,
};
