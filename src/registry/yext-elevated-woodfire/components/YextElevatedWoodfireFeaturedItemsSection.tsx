...
type ResolvedVisualImageProps = Omit<VisualImageProps, "image"> & {
  image?:
    | VisualImageProps["image"]
    | ImageType
    | ComplexImageType
...
const featuredItemsSource = createItemSource<FeatureItemSourceProps>({
  label: "Items",
  mappingFields: {
    title: {
      type: "entityField",
    ...
    description: rtfField("Description") as any,
    image: {
      label: "Image",
      type: "object",
  ...
  defaultValues: [
    {
      title: {
        field: "",
        constantValue: "Redwood Smokehouse Burger",
        constantValueEnabled: true,
      },
    ...
    },
    {
      title: {
        field: "",
        constantValue: "South Lamar Chicken Sandwich",
        constantValueEnabled: true,
      },
      description: defaultRtf(
    ...
    },
    {
      title: {
        field: "",
        constantValue: "Hill Country Steak Salad",
        constantValueEnabled: true,
      },
      description: defaultRtf(
      ...
      image: defaultImage(`${imageBaseUrl}salad.jpg`, 0, "filled", 5184, 3456),
      cta: {
        label: {
          field: "",
          constantValue: "Learn More",
...
const faqsSource = createItemSource<FaqItemSourceProps>({
  label: "Items",
  mappingFields: {
    question: {
      type: "entityField",
  ...
  defaultValues: [
    ...
    },
    {
      question: {
        field: "",
        constantValue: "Do you have outdoor seating?",
        constantValueEnabled: true,
      },
      answer: defaultRtf(
    ...
    },
    {
      question: {
        field: "",
        constantValue: "Do you offer happy hour specials?",
        constantValueEnabled: true,
      },
      answer: defaultRtf(
...
const renderRtf = (
  text: Omit<StyledRtfProps, "text"> & {
    text?: StyledRtfProps["text"] | TranslatableRichText;
  },
  locale: string,
  streamDocument: StreamDocumentWithReviews,
  className?: string,
  ...
  <div className={className} style={textStyle(text)}>
    {renderResolvedRichText(
      typeof text.text === "object" &&
        text.text !== null &&
        ("field" in text.text || "constantValue" in text.text)
...
const renderImage = (
  image: ResolvedVisualImageProps,
  locale: string,
  streamDocument: StreamDocumentWithReviews,
  ...
  const resolvedImage =
    ...
    image.image !== null &&
    ("field" in image.image || "constantValue" in image.image)
      ? resolveComponentData(image.image, locale, streamDocument)
      : image.image;
  if (!resolvedImage) {
    return null;
  }
...
> = (props) => {
  ...
  const featuredItems = featuredItemsSource.resolveItems(
    props.featuredItems.items,
    streamDocument,
  );
  const faqItems = faqsSource.resolveItems(props.faqs.items, streamDocument);
...
const pickYextElevatedWoodfireDefaultProps = <
  K extends YextElevatedWoodfireEditableKey,
>(
  keys: readonly K[],
...
`;
export const YextElevatedWoodfireFeaturedItemsSection =
  createYextElevatedWoodfireSection(
    "Yext Elevated Woodfire Featured Items Section",
    "featured",
